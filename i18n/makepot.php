...
class MakePOT {
	...
	public function wp_plugin( $dir, $output, $slug = null, $args = array() ) {
		...
		/*
		 * wp_plugin_split function added by C. Seguinot to split plugin pot file into common admin frontend ... parts
		 * whithout modifying the rest of the original makepot.php code.
		*/

		public function wp_plugin_split( $dir, $output, $slug = null, $args = array() ) {
		...
		$default = array(
			...
			'includes' => array(),
                ...
                // Load plugin makepot args
		$makepot_args = array(); 
		$makepot_args_file = rtrim($dir, '/') . '/i18n/makepot_args.php';
		if (file_exists( $makepot_args_file ) )  include $makepot_args_file;
                
                // Is $args no longer usefull !! 
                $makepot_args = array_merge($default, $makepot_args);
		...
		// Create each (or one) pot file(s) from $makepot_args
		$res = false; 
		$pot_files = array(); 
		$common_pot = false;
		
		foreach ($makepot_args as $key => $args) {
                    $is_standalone = ( ($key!='common') AND ($key!='frontend') AND ($key!='admin') );
                    $use_for_common = (! $is_standalone) ? true : (isset($args['use_for_common']) AND $args['use_for_common']==true)? true : false;
                    $output = (isset($args['output']) AND $args['output']!='') ? $args['output'] : $slug. 
                        ((isset($args['slug_extension']) AND $args['slug_extension']!='') ? '-' :'') . (string)$args['slug_extension'] .'.pot'; 
                
                    if ($is_standalone) {
                        // Do not include unnecessary author name and slug from standalone pot files
                        $this_pot_exists = $this->xgettext( 'wp-plugin', $dir, $output, array(), $args['excludes'], $args['includes'] );
                    } else {
                        $this_pot_exists = $this->xgettext( 'wp-plugin', $dir, $output, $placeholders, $args['excludes'], $args['includes'] );
                        $potextmeta = new PotExtMeta;
                        $potextmeta->append($main_file, $output);
                        /* Adding non-gettexted strings can repeat some phrases */
                        $output_shell = escapeshellarg($output);
                        system("msguniq $output_shell -o $output_shell");
                    ...
                    if ($this_pot_exists) {
                        if ( $key =='common' ) $common_pot = $output;
                        $pot_files[$output] = array ('is_standalone' => $is_standalone, 'use_for_common' => $use_for_common); 
                        // At least one pot file exists
                        $res = true;
                    }
                }     
		if (count($pot_files) <=1 ) return $res;
 
                // Iterate on each couple of pot file to process common phrases
		$pot_files2 = $pot_files;
		$processed = array(); 
		foreach ($pot_files as $pot_file1 => $pot_file1_args) {
                    foreach ($pot_files2 as $pot_file2 => $pot_file2_args) {
                        $processed[] = $pot_file1.$pot_file2;
                        
                        if (($pot_file1 != $pot_file2) AND $pot_file1_args['use_for_common']==true AND true==$pot_file2_args['use_for_common'] AND (! array_key_exists($pot_file2.$pot_file1, $processed)))  {
                            // Empty previous existing temp.pot if any 
                            system( "> temp.pot" ); // TODO Only for Unix and Linux based system
                            // Process these 2 files for common phrases
                            system( "msgcat --more-than=1 --use-first $pot_file1 $pot_file2 -o temp.pot" );
                            //remove common phrases from non standalone and non common.pot files
                            if (($pot_file1 !=$common_pot) AND (false == $pot_file1_args['is_standalone'])) {
                                system( "msgcat -u --use-first $pot_file1 temp.pot -o $pot_file1" );
                            }
                            if (($pot_file2 !=$common_pot) AND (false == $pot_file2_args['is_standalone'])) {
                                system( "msgcat -u --use-first $pot_file2 temp.pot -o $pot_file2" );
                            }   
                            // Include common phrases in common.pot 
                            system( "msgcat --use-first temp.pot $common_pot -o $common_pot" );
                        }
                    }
                }
                system( "rm temp.pot" ); // TODO Only for Unix and Linux based system
                return $res;      
	}
